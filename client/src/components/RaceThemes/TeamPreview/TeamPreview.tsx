...
import { a, useChain, useSpring, useSpringRef, useTrail, useTransition } from "react-spring";
...
function TeamPreview(props: Props) {
    ...
    const subtitleAnimation = useSpring({
        ...
        from: { opacity: 0 },
        to: { opacity: 1 },
        onRest: () => setTimeout(() => setShowMainTeam(curr => true), 15000)
    })
    ...
    const teamTransitionRef = useSpringRef()
    const teamTransition = useTransition(sortedTeams, {
        ref: teamTransitionRef,
    ...
    useChain([titleAnimationRef, subtitleAnimationRef, teamContainerAnimationRef, teamTransitionRef], [0, 0.1, 0.2, 0.25], 10000)
            ...
            <motion.div className="page-content" initial={{ filter: "blur(0px)" }} animate={{ filter: startCountdown ? "blur(2px)" : "blur(0px)"}}>
                                ...
                                <div className="team-container">
                                    ...
                                    <div className="team-preview-name-container">
                                        <div style={{textDecorationLine: "underline", textDecorationColor: ghost.colors.highlightColor}}>
                                            <span>{ghost.study + ":"}</span>{ghost.teamName}
                                        </div>
                                    </div>
                                </div>
                            </a.div>
                ...
                {showMainTeam ? (
                    <a.div className="main-team-preview" style={mainTeamAnimation}>
                        <div className="main-team-image-container">
